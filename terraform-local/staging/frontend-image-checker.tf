        ...
        template {
          ...
          spec {
            container {
              ...
              command = ["/bin/bash", "-c", "apt update\napt install -y curl\nDEPLOYMENT_NAME=fe-angular-game-price-comparator-staging-deployment\nDOCKER_IMAGE=\"derreiskanzler/fe-angular-game-price-comparator-staging\"\nDOCKER_TAG=\"latest\"\nNAMESPACE=default\nCURRENT_DIGEST=$(kubectl get pod -l app=fe-angular-game-price-comparator-staging -o=jsonpath='{.items[0].status.containerStatuses[0].imageID}' | cut -d'@' -f2)\nLATEST_DIGEST=$(curl -s https://hub.docker.com/v2/repositories/$${DOCKER_IMAGE}/tags/$${DOCKER_TAG}/ | grep -o '\"digest\":\"[^\"]*')\n\necho --------\necho Current Digest\necho $CURRENT_DIGEST\necho Latest digest\necho $LATEST_DIGEST\necho --------\n\nif [[ \"$LATEST_DIGEST\" != *\"$CURRENT_DIGEST\"* ]]; then\n  echo \"New image version found. Updating the deployment...\"\n  kubectl rollout restart deployment/$DEPLOYMENT_NAME -n $NAMESPACE\n  echo \"Deployment updated successfully.\"\nelse\n  echo \"No new image version found.\"\nfi\n"]
